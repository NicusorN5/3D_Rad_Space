...
{
    [DllImport("3DRadSpace.FFI.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "E3DRSP_AudioBuffer_Create")]
    private static extern IntPtr create(IntPtr buffer, int numChannels, int sampleRate, int bps, int format, int size);

    [DllImport("3DRadSpace.FFI.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "E3DRSP_AudioBuffer_FromWAV")]
    private static extern IntPtr fromWAV(string path);

    [DllImport("3DRadSpace.FFI.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "E3DRSP_AudioBuffer_FromOGG")]
    private static extern IntPtr fromOGG(string path);

    [DllImport("3DRadSpace.FFI.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "E3DRSP_AudioBuffer_Destroy")]
